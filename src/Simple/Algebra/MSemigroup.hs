...
-- recover the usual mathematical operators we want (addition, multiplication)
...
instance (Eq a, Num a, Ord a) => MSemigroup (NonNegative a) where
  MkNonNegative x .*. MkNonNegative y = unsafeNonNegative $ x * y

instance (Eq a, Num a, Ord a) => MSemigroup (Positive a) where
